...
for (i in falls) {
  print(i)
time<-seq(from = 1/Fs, to =2 , by = 1/Fs)
zeros<-matrix(0, dim(Falls_df)[1], 1)
zeros[seq(from = fall_ind[i]-100, to =fall_ind[i]+99 , by = 1)] =1
ind<-as.logical(zeros)
f_point<-matrix(0, Fs*2, 1)
f_point[101] = 1
...
geom_line(linetype = "solid") + labs(title = "X Acceleration")+ geom_point(data=df, aes(x = time[f_point], y=acX[f_point], group=2),color = 'red')

py<-ggplot(data=df, aes(x = time, y=acY, group=1)) +
geom_line(linetype = "solid") + labs(title = "Y Acceleration")+ geom_point(data=df, aes(x = time[f_point], y=acY[f_point], group=2),color = 'red')

pz<-ggplot(data=df, aes(x = time, y=acZ, group=1)) +
geom_line(linetype = "solid") + labs(title = "Z Acceleration")+ geom_point(data=df, aes(x = time[f_point], y=acZ[f_point], group=2),color = 'red')
print(px + py + pz)

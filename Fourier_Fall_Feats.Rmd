...
roll_off <- function(a){
  amp<-Mod(fft(a))/length(a)
  freq<-1:length(amp)
  freq<-freq/(length(amp)/100)
  freq<-freq[1:round(length(amp))/2]
  amp<-amp[1:round(length(amp))/2]
  cums<-cumsum(amp)
  roll<-cums<0.85*max(cums)
  rolloff<-tail(freq[roll],1)
  return(rolloff)}

spec_centroid <- function(a){
  amp<-Mod(fft(a))/length(a)
  freq<-1:length(amp)
  freq<-freq/(length(amp)/100)
  freq<-freq[1:round(length(amp))/2]
...
roll_off(accX)
rolloff_accX <- SlidingWindow("roll_off", abs(Train_df$accX), window, step)
```
```{r}
spec_centroid(accX)
specCen_accX <- SlidingWindow("spec_centroid", abs(Train_df$accX), window, step)
...
L2(accX)
L2_accX <- SlidingWindow("L2", abs(Train_df$accX), window, step)

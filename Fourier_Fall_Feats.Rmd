...
Data<-data.frame(Target,accX,accY,accZ,angX,angY,angZ,magX,magY,magZ)
```
...
feats<-c('entropy','mean_energy','total_energy','min','max','q1','q3','sd','skewness','kurtosis','median','max_diff','rms')
Feat_df<-feature_gen(Data,feats,window = window,step =step)
...
```{r}
Falls_df <- subset(Feat_df, Target==1)          
noFalls_df<- subset(Feat_df, Target==0)
set.seed(97)
## 80% of the sample size
smp_size <- floor(0.80 * nrow(Falls_df))
falls_train_ind <- sample(seq_len(nrow(Falls_df)), size = smp_size)
...
```{r}
trainRows <- sample(nrow(Train_df))
testRows <- sample(nrow(Test_df))
Train_feat_df<-Train_df[trainRows,]
Test_feat_df<-Test_df[testRows,]
```

```{r}
setwd("~/Desktop/Duke/Class/Fall 2020/Biomedical DS/BME590_Fall_Detection/")
write.csv(Train_feat_df,'Train_df_V2.csv')
write.csv(Test_feat_df,'Test_df_V2.csv')

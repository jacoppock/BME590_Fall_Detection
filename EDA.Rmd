...
Falls_df_rms <- mutate(Falls_df_rms, acc = (Falls_df_rms$accX^2 + Falls_df_rms$accY^2 + Falls_df_rms$accZ^2)^0.5)
Falls_df_rms <- mutate(Falls_df_rms, ang = (Falls_df_rms$angX^2 + Falls_df_rms$angY^2 + Falls_df_rms$angZ^2)^0.5)
Falls_df_rms <- mutate(Falls_df_rms, mag = (Falls_df_rms$maxX^2 + Falls_df_rms$magY^2 + Falls_df_rms$magZ^2)^0.5)

Falls_df_rms <- select(Falls_df_rms, Time, acc, ang, mag, Target)
...
AllFeaturesRMS <- data.frame(Targets,`mean_Falls_df_rms$acc`,`max_Falls_df_rms$acc`,`min_Falls_df_rms$acc`,`median_Falls_df_rms$acc`,`q1_Falls_df_rms$acc`,
                                    `q3_Falls_df_rms$acc`,`rms_Falls_df_rms$acc`,`std_Falls_df_rms$acc`,`skew_Falls_df_rms$acc`,`kurtosis_Falls_df_rms$acc`,
                                     `mean_Falls_df_rms$ang`,`max_Falls_df_rms$ang`,`min_Falls_df_rms$ang`,`median_Falls_df_rms$ang`,`q1_Falls_df_rms$ang`,
                                    `q3_Falls_df_rms$ang`,`rms_Falls_df_rms$ang`,`std_Falls_df_rms$ang`,`skew_Falls_df_rms$ang`,`kurtosis_Falls_df_rms$ang`,
                                     `mean_Falls_df_rms$mag`,`max_Falls_df_rms$mag`,`min_Falls_df_rms$mag`,`median_Falls_df_rms$mag`,`q1_Falls_df_rms$mag`,
                                    `q3_Falls_df_rms$mag`,`rms_Falls_df_rms$mag`,`std_Falls_df_rms$mag`,`skew_Falls_df_rms$mag`,`kurtosis_Falls_df_rms$mag`)
...
names(AllFeaturesRMS)[8] <- "RMS(acc)"
names(AllFeaturesRMS)[9] <- "SD(acc)"
names(AllFeaturesRMS)[10] <- "Skew(acc)"
names(AllFeaturesRMS)[11] <- "Kurtosis(acc)"

names(AllFeaturesRMS)[12] <- "Mean(ang)"
names(AllFeaturesRMS)[13] <- "Max(ang)"
names(AllFeaturesRMS)[14] <- "Min(ang)"
names(AllFeaturesRMS)[15] <- "Median(ang)"
names(AllFeaturesRMS)[16] <- "Q1(ang)"
names(AllFeaturesRMS)[17] <- "Q3(ang)"
names(AllFeaturesRMS)[18] <- "RMS(ang)"
names(AllFeaturesRMS)[19] <- "SD(ang)"
names(AllFeaturesRMS)[20] <- "Skew(ang)"
names(AllFeaturesRMS)[21] <- "Kurtosis(ang)"

names(AllFeaturesRMS)[22] <- "Mean(mag)"
names(AllFeaturesRMS)[23] <- "Max(mag)"
names(AllFeaturesRMS)[24] <- "Min(mag)"
names(AllFeaturesRMS)[25] <- "Median(mag)"
names(AllFeaturesRMS)[26] <- "Q1(mag)"
names(AllFeaturesRMS)[27] <- "Q3(mag)"
names(AllFeaturesRMS)[28] <- "RMS(mag)"
names(AllFeaturesRMS)[29] <- "SD(mag)"
names(AllFeaturesRMS)[30] <- "Skew(mag)"
names(AllFeaturesRMS)[31] <- "Kurtosis(mag)"
...
x %>% 
  ...
  ggplot(aes(x = rowname, y = Targets)) +
    geom_bar(stat = "identity") +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ggtitle("Correlation of Falls (Label) with Features") +
    geom_bar(aes(fill = Targets < 0), stat = "identity") +   scale_fill_manual(guide = FALSE, breaks = c(TRUE, FALSE), values=c("#BB4444", "#4477AA")) +
    labs(x = "Feature", y = "Pearson Correlation") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),

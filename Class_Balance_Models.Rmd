...
train_perf_rr <- predict(model_rr, s = 1e100, 
                         newx = train_x, type = "response")
ROC_train_rr <- rocit(score=as.vector(train_rr), class=train_y)
...
test_perf_rr <- predict(model_rr, s = 1e100, 
...
model_svm <- tune.svm(Target~., data = train_df, 
                      cost = 2e-2, 
                      kernel = "poly",tunecontrol=tune.control(cross=10),
                      probability=TRUE)
summary(model_svm)
plot(model_svm)
...
train_perf_svm <- predict(model_svm$best.model, train_df[,1:ncol(train_df)], probability=TRUE)
...
test_perf_svm <- predict(model_svm$best.model, test_df[,1:ncol(test_df)], probability=TRUE)
...
plot(ROC_test_svm, legend = TRUE, YIndex = FALSE, values = TRUE)
...
model_rf <- tune.randomForest(Target~., data = train_df, 
                      ...
                      ntree = c(1,10,100,1000,10000), probability=TRUE, 
                      importance=TRUE)
summary(model_rf)
...
model_rf$performances
train_perf_rf <- predict(model_rf$best.model, train_df[,1:ncol(train_df)], type="prob")
...
test_perf_rf <- predict(model_rf$best.model, test_df[,1:ncol(test_df)], type="prob")
